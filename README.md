...
* `chooser` - объект настройки выборщика постов
    * `name` - может содержать одно из встроенных значений:
        * `mostRated` - выборка по нисходящей рейтинга от самых старых к новым
        * `mostRatedRandom` - выборка случайных из самых высокорейтинговых. То есть
        если существует 4 поста с рейтингом 6, выбран будет случайный, а не самый
        старый, как в предыдущем варианте
        * `smartChooser` - на данный момент самый сложный и гибкий из встроенных
        объектов выборщика.
        * `none` - всегда ничего не выбирает :\)
    * `params` - объект настройки для выборщика. Далее приведены инструкции по
    настройке встроенных выборщиков:
        * `smartChooser` - имеет следующую структуру для параметра `params`
        объекта `chooser`:
